    ...
    attach: function (context) {
      $('body').once('official-facebook-pixel').each(function () {
        ...
        if (drupalSettings.official_facebook_pixel.pixel_id > 0 && !Drupal.official_facebook_pixel.disabled) {
          ...
          var debug = drupalSettings.official_facebook_pixel.debug;
          ...
          if (privacy_fb_optout) {
            ...
            window.fbOptout = function(reload) {
              ...
              if(debug){
                console.debug('official_facebook_pixel: ' + 'Tracking disabled: Opted-out from offical_facebook_pixel by fbOptout()');
              }
              if (reload) {
                location.reload();
              }
            ...
            if (document.cookie.indexOf(privacy_fb_optout_key + '=true') !== -1) {
              window[privacy_fb_optout_key] = true;
              if(debug){
                console.debug('official_facebook_pixel: ' + 'Tracking disabled: Opted-out from offical_facebook_pixel by cookie: "' + privacy_fb_optout_key + '"');
              }
            }
          ...
          if (privacy_eu_cookie_compliance) {
            ...
            if(debug && !eccHasAgreed){
              console.debug('official_facebook_pixel: ' + 'Tracking disabled: eu_cookie_compliance integration enabled, Drupal.eu_cookie_compliance.hasAgreed() returned FALSE.');
            }
          ...
          if (privacy_donottrack) {
            ...
            if(debug){
              console.debug('official_facebook_pixel: ' + 'Tracking disabled: DNT was sent by your client.');
            }
          ...
          if (!Drupal.official_facebook_pixel.disabled) {
            ...
            if(debug){
              console.debug('official_facebook_pixel: ' + 'Initialized.');
            }
            // Run events:
            events.forEach(function (item, index) {
              Drupal.official_facebook_pixel.executeEvent(item);
              if(debug){
                console.debug('official_facebook_pixel: ' + 'Executed event: ', item);
              }
            });
          }
        }
